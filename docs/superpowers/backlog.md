...
- **Multi-kind effort per entry (schema v6).** `Entry.effort: EffortItem[]` replaces the scalar `effort_kind` + `effort_count` pair. Chip-list UI on Log + Edit with inline add-row (kind dropdown + count + Add). Three-layer uniqueness-by-kind enforcement (validator clones/normalizes, upgrader normalizes on read, writer probe asserts). Golden-hash-stable canonicalization (empty effort omitted from canonical form). CSV export collapses two columns into one `effort` column with `kind:count;kind:count` sorted by kind. Commit messages surface effort as `[activity: 2 meeting, 1 slack]` suffix.
