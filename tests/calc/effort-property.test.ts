...
/**
 * Entry arbitrary with correctly-coupled effort fields (both null or both
 * set). Enforces the cross-field rule so the calc sees realistic inputs.
 */
  ...
  fc
    .record({
      ...
      effort_kind: fc.oneof(
        ...
        fc.constantFrom(...ALL_EFFORT_KINDS),
      ),
      effort_count: fc.integer({ min: 1, max: 100 }),
    })
    .map((e) => {
      // Enforce cross-field rule on the fly.
